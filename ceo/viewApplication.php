...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'approve' && $appId > 0) {
    try {
        $stmt = $pdo->prepare('SELECT status_id FROM applications WHERE id = ?');
        $stmt->execute([$appId]);
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($row && (int) $row['status_id'] === 6) {
            $pdo->beginTransaction();
            $pdo->prepare('UPDATE applications SET status_id = 3 WHERE id = ?')->execute([$appId]);
            $pdo->prepare('INSERT INTO application_history (application_id, from_status_id, to_status_id, staff_id, action, notes) VALUES (?, 6, 3, ?, ?, NULL)')
                ->execute([$appId, $ceoId ?: null, 'ceo_approve']);
            $pdo->commit();
            $msg = 'success';
        } else {
            $msg = 'invalid';
    ...
    header('Location: viewApplication.php?id=' . $appId . '&m=' . $msg);
    exit;
}
...
if ($appId > 0) {
    try {
        ...
        if ($row) {
            ...
            $statusName = $row['status_name'] ?? 'Committee Approved';
        }
        if ($app) {
...
<head>
    ...
    <title><?php echo $app ? 'Application #' . $appId : 'Application'; ?> — RCMP UniFa CEO</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .badge--committee_approved { background: #dbeafe; color: #1e40af; }
        ...
        .btn-approve { padding: 0.6rem 1.5rem; border-radius: 10px; background: #166534; color: #fff; font-size: 0.9rem; font-weight: 600; border: none; cursor: pointer; }
    ...
    <div class="app">
        <aside class="sidebar">
            ...
            <nav class="sidebar-nav">
                ...
                <a href="application.php" class="active"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>Pending approval</a>
                <a href="../admin/history.php"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>History</a>
        ...
        <div class="main-content">
            ...
            <a href="application.php" class="btn-back">← Back to Pending</a>

            <?php if ($msg === 'success'): ?><div class="flash flash-success">Application approved. Admin can now upload receipt.</div><?php endif; ?>
            <?php if ($msg === 'error'): ?><div class="flash flash-error">Could not update. Please try again.</div><?php endif; ?>

            <?php if (!$app): ?>
                <div class="not-found">Application not found or not pending CEO approval.</div>
            <?php else: ?>
                <div class="card">
                    <h3>Status</h3>
                    <span class="badge badge--committee_approved"><?php echo fmt($statusName); ?></span>
                </div>
                <div class="card">
