...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($full_name === '' || $email === '') {
        ...
        try {
            $stmt = $pdo->prepare('UPDATE staff SET full_name = ?, email = ?, phone = ? WHERE id = ? AND role = 3');
            $stmt->execute([$full_name, $email, $phone, $ceoId]);
            $_SESSION['user_name'] = $full_name;
            $_SESSION['user_email'] = $email;
            $profile_success = 'Profile updated successfully.';
        } catch (PDOException $e) {
            $profile_error = 'Could not update profile. Email may already be in use.';
        }
    }
}
...
$user = null;
try {
    $stmt = $pdo->prepare('SELECT staff_id, full_name, email, phone, created_at FROM staff WHERE id = ? AND role = 3 LIMIT 1');
    $stmt->execute([$ceoId]);
    $user = $stmt->fetch(PDO::FETCH_ASSOC);
} catch (PDOException $e) {}
if (!$user) {
    $user = [
        'staff_id' => $_SESSION['ceo_staff_id'] ?? '—',
        'full_name' => $_SESSION['user_name'] ?? 'CEO',
...
<head>
    ...
    <title>Profile — RCMP UniFa CEO</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .profile-body input { width: 100%; border-radius: 10px; border: 1px solid #e5e7eb; padding: 0.6rem 0.85rem; font-size: 0.9rem; outline: none; background: #fff; transition: border-color 0.15s; font-family: inherit; }
        .profile-body input:focus { border-color: #4f46e5; }
        .profile-body input[readonly] { background: #f9fafb; color: #6b7280; cursor: default; }
    ...
    <div class="app">
        <aside class="sidebar">
            ...
            <nav class="sidebar-nav">
                ...
                <a href="../ceo/application.php"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>Applications</a>
                <a href="../ceo/profile.php" class="active"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>Profile</a>
            </nav>
            ...
            <div class="profile-card">
                <div class="profile-header">
                    ...
                    <div class="profile-header-text">
                        ...
                        <p>CEO since <?php echo htmlspecialchars($memberSince); ?></p>
                    </div>
                </div>
                <div class="profile-body">
                    <div class="profile-row">
                        <span class="profile-label">Staff ID</span>
                        <input type="text" value="<?php echo htmlspecialchars($user['staff_id'] ?? ''); ?>" readonly>
                    </div>
                    <div class="profile-row">
                        <span class="profile-label">Full name</span>
                        <input type="text" name="full_name" value="<?php echo htmlspecialchars($user['full_name'] ?? ''); ?>" placeholder="Your full name" required>
                    </div>
                    <div class="profile-row">
                        <span class="profile-label">Email</span>
                        <input type="email" name="email" value="<?php echo htmlspecialchars($user['email'] ?? ''); ?>" placeholder="your@email.com" required>
                    </div>
                    <div class="profile-row">
