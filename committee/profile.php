...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['pw_current'])) {
    ...
    if ($full_name === '' || $email === '') {
        ...
        try {
            $stmt = $pdo->prepare('UPDATE staff SET full_name = ?, email = ?, phone = ? WHERE id = ? AND role = 2');
            $stmt->execute([$full_name, $email, $phone, $committeeId]);
...
$user = null;
try {
    $stmt = $pdo->prepare('SELECT staff_id, full_name, email, phone, created_at FROM staff WHERE id = ? AND role = 2 LIMIT 1');
    $stmt->execute([$committeeId]);
    $user = $stmt->fetch(PDO::FETCH_ASSOC);
} catch (PDOException $e) {}
if (!$user) {
    $user = [
        'staff_id' => $_SESSION['committee_staff_id'] ?? '—',
        'full_name' => $_SESSION['user_name'] ?? 'Committee',
            ...
            <div class="profile-card">
                <div class="profile-header">
                    ...
                    <div class="profile-header-text">
                        ...
                        <p>Committee member since <?php echo htmlspecialchars($memberSince); ?></p>
                    </div>
                </div>
