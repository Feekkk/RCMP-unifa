...
$committeeStaffId = (int) ($_SESSION['committee_id'] ?? 0);
$appId = (int) ($_GET['id'] ?? 0);
$msg = $_GET['m'] ?? '';
$app = null;
$user = null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $appId > 0) {
    $action = trim((string) ($_POST['action'] ?? ''));
    $notes = trim((string) ($_POST['notes'] ?? ''));
    $valid = in_array($action, ['approve', 'reject']);
    if ($valid && $action === 'reject' && $notes === '') {
        $valid = false;
        $msg = 'reject_notes';
    }
    if ($valid) {
        try {
            ...
            if ($row && (int) $row['status_id'] === 2) {
                ...
                $pdo->prepare('INSERT INTO application_history (application_id, from_status_id, to_status_id, staff_id, action, notes) VALUES (?, 2, ?, ?, ?, ?)')
                    ->execute([$appId, $toStatus, $committeeStaffId ?: null, $action, $notes ?: null]);
                $pdo->commit();
                $msg = 'success';
            } else {
                $msg = 'invalid';
...
if ($appId > 0) {
    try {
        ...
        if ($row) {
            ...
            $statusName = $row['status_name'] ?? 'Under Review';
        }
...
<head>
    ...
    <title><?php echo $app ? 'Application #' . $appId : 'Application'; ?> — RCMP UniFa Committee</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .action-card textarea { width: 100%; border-radius: 10px; border: 1px solid #e5e7eb; padding: 0.65rem 0.85rem; font-size: 0.9rem; min-height: 80px; resize: vertical; margin-bottom: 0.75rem; }
        .action-card .btn-group { display: flex; gap: 0.75rem; flex-wrap: wrap; }
        .action-card .btn-approve { padding: 0.6rem 1.25rem; border-radius: 10px; background: #22c55e; color: #fff; font-size: 0.9rem; font-weight: 600; border: none; cursor: pointer; }
        .action-card .btn-approve:hover { background: #16a34a; }
        .action-card .btn-reject { padding: 0.6rem 1.25rem; border-radius: 10px; background: #dc2626; color: #fff; font-size: 0.9rem; font-weight: 600; border: none; cursor: pointer; }
        .action-card .btn-reject:hover { background: #b91c1c; }
...
<body>
    ...
    <div class="app">
        ...
        <div class="main-content">
            ...
            <?php if ($msg === 'success'): ?><div class="flash flash-success">Decision saved successfully.</div><?php endif; ?>
            <?php if ($msg === 'error'): ?><div class="flash flash-error">Could not save. Please try again.</div><?php endif; ?>
            <?php if ($msg === 'reject_notes'): ?><div class="flash flash-error">Notes are required when rejecting.</div><?php endif; ?>

            <?php if (!$app): ?>
                <div class="not-found">Application not found or no longer under review.</div>
            <?php else: ?>
                <div class="card">
                    <h3>Status</h3>
                    <span class="badge badge--under_review"><?php echo fmt($statusName); ?></span>
                </div>
                ...
                <div class="card action-card">
                    ...
                    <form method="post" action="" id="decisionForm">
                        <label for="notes" class="info-label" style="display:block;margin-bottom:0.35rem">Notes <span style="color:#dc2626">(required for reject)</span></label>
                        <textarea name="notes" id="notes" placeholder="Add notes (required when rejecting)"></textarea>
                        <div class="btn-group">
